...
public class SeedorderHistory extends AppCompatActivity {

    FirebaseFirestore firestore=FirebaseFirestore.getInstance();
    RecyclerView recyclerView,recyclerView2;
    ArrayList<SeedOrderModel> orderlist=new ArrayList<>();
    ArrayList<SeedOrderModel> orderlistpast=new ArrayList<>();
    SeedOrderAdapter adapter,adapter1;
    FirebaseAuth mauth;
    String farmerid;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_seedorder_history);
        ...
        adapter=new SeedOrderAdapter(this,orderlist);
        adapter1=new SeedOrderAdapter(this,orderlistpast);

        recyclerView.setAdapter(adapter);
        recyclerView2.setAdapter(adapter1);


        firestore.collection("Farmers").document(farmerid).collection("Orders").whereEqualTo("Order_Status","Delivered").get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    ...
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        if(!queryDocumentSnapshots.isEmpty()){
                            ...
                            Toast.makeText(SeedorderHistory.this, "No data found", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    ...
    private void getOrders() {
        firestore.collection("Farmers").document(farmerid).collection("Orders").whereEqualTo("Order_Status","Order Placed").get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    @Override
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        if(!queryDocumentSnapshots.isEmpty()){
